...
# Part XV — 멀티에이전트 오케스트레이션 (규모 확장 시)

> 본 Part 는 **원리 6 (병행 가능성 우선, Part II)** 의 *실천편*이다. 1~3인 + AI 협업이 *여러 에이전트*로 확장될 때, "여러 에이전트를 어떻게 동시에 굴리고·통신시키고·검증하는가" 를 다룬다.

## 15.1 언제 도입하나

단일 에이전트(한 세션)로는 *작업량이 많거나·서로 다른 전문성*이 필요할 때, 같은 작업을 *대량 병렬*로 돌릴 때, 또는 결과를 *적대적으로 검증*해야 신뢰가 서는 곳. 1인 단일 세션이면 오버헤드만 늘어나므로 미적용.
...
|---|---|---|---|
| **층** | 설계 층 (실행환경) | 협업·관찰 층 | 대량 병렬·검증 층 |
| **핵심** | 도구·skill·프롬프트·진입점 큐레이션 | 에이전트 간 메시지·관찰 | 스크립트로 수십 에이전트 결정적 제어 |
| **통신** | 없음 (환경만) | 있음 (메시지) | 없음 (단계 산출물로 전달) |
| **규모** | 1 에이전트 = 1 환경 | 소수 (2~6) | 대량 (수십~수백) |
| **언제** | *환경을 짜야 할 때* (항상 선행) | *에이전트끼리 메시지가 필요할 때* | *대량 독립 작업 + 결정적 제어 + 엄격 검증* |

분기 한 줄 — **서로 메시지가 필요하면 Teams, 대량·독립·검증이면 Workflows, 그 전에 환경은 하네스.**

## 15.3 모듈 포인터

상세는 `tier3/docs/orchestration/` 모듈에 위임한다.

| 문서 | 다루는 것 |
|---|---|
| `orchestration/README.md` | 3 모드 구분표 + "언제 무엇을 쓰나" 의사결정 가이드 |
| `orchestration/harness_engineering.md` | 🛠 에이전트별 실행환경 · 라이프사이클 3종 · 진화 델타 |
| `orchestration/agent_teams.md` | 👥 TeamCreate · 통신 · tmux 분할(총 pane 수 기준) · 표준 구성 |
| `orchestration/dynamic_workflows.md` | 🟣 JS 오케스트레이션 · pipeline vs parallel · 적대검증 |
| `orchestration/team_spawn_playbook.md` | 팀 spawn 풀 라이프사이클 · 산출물 규약 · 회고 루프 |

> Tier 3 모듈이다. Tier 1·2 만으로 충분한 단계에서는 미적용 — 단일 세션으로 충분하면 멀티에이전트는 비용일 뿐이다.
...
| 2026-05-30 | v0.4 | Part XV (멀티에이전트 오케스트레이션 — 원리 6 실천편) 추가. tier3/docs/orchestration/ 모듈 5종 신설 (README·하네스 엔지니어링·Agent Teams·Dynamic Workflows·팀 spawn 플레이북). 3 실행 모드 구분표·산출물 규약·회고 루프 정식 편입. |
